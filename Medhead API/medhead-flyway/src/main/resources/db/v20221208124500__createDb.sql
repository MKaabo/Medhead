...
CREATE TABLE `doctor` (
	...
	`specialization` ENUM('Y','N') NULL DEFAULT NULL COLLATE 'latin1_swedish_ci',
	`hospitalId` INT(11) NULL DEFAULT NULL,
	PRIMARY KEY (`id`) USING BTREE,
	INDEX `doctor_hospital` (`hospitalId`) USING BTREE,
	CONSTRAINT `doctor_hospital` FOREIGN KEY (`hospitalId`) REFERENCES `api medhead`.`hospital` (`id`) ON UPDATE CASCADE ON DELETE CASCADE
)
